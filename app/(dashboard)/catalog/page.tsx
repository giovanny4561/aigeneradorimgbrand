...
const INITIAL_PRODUCTS: Product[] = [
  {
    ...
    imageNoBg: '', visualReferences: [], status: 'Ready', tags: ['Calzado', 'Verano']
  },
  {
    id: '2', name: 'Eco Succulent',
    description: 'Mini suculenta artificial en maceta de cerámica reciclada, perfecta para escritorios modernos.',
    price: 24.50, sku: 'PLT-MINI-04',
    ...
    imageNoBg: '', visualReferences: [], status: 'Processing', tags: ['Decoración']
  },
  {
    id: '3', name: 'Sonic Bass Pro',
    description: 'Auriculares inalámbricos con cancelación de ruido activa y 30 horas de batería.',
    price: 199.00, sku: 'AUD-HEAD-99',
    ...
    imageNoBg: '', visualReferences: [], status: 'Ready', tags: ['Electrónica', 'Audio']
  }
]
...
export default function CatalogPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>, field: 'image' | 'imageNoBg') => {
  ...
  const handleAddReference = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (res.ok) {
        const { url } = await res.json()
        setFormData((prev) => ({ ...prev, visualReferences: [...(prev.visualReferences || []), url] }))
      }
    } catch {
      const url = URL.createObjectURL(file)
      setFormData((prev) => ({ ...prev, visualReferences: [...(prev.visualReferences || []), url] }))
    ...
    <div className="flex-1 flex flex-col h-full bg-slate-50 relative overflow-hidden">
      ...
      <header className="flex h-16 items-center justify-between border-b border-slate-200 px-8 bg-white/80 backdrop-blur-md sticky top-0 z-20">
        ...
        <div className="flex items-center gap-4">
          <button onClick={() => router.push('/strategy')} className="flex items-center gap-2 rounded-lg bg-primary px-4 py-2 text-sm font-bold text-white shadow-sm hover:bg-primary-hover transition-all">
            <span>Continuar a Estrategia</span>
              ...
              <div key={product.id} className="group relative flex flex-col overflow-hidden rounded-2xl bg-white border border-slate-200 shadow-sm hover:border-primary/50 hover:shadow-lg hover:shadow-primary/5 transition-all duration-300">
                <div className="relative aspect-square w-full overflow-hidden p-6 flex items-center justify-center border-b border-slate-100 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PHBhdGggZD0iTTEwIDBoMTB2MTBIMTBWMHkwIDEwaDEwdjEwSDBWMHoiIGZpbGw9IiNmOGZhZmMiIGZpbGwtcnVsZT0iZXZlbm9kZCIvPjwvc3ZnPg==')]">
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2 backdrop-blur-[2px]">
                    <button onClick={(e) => { e.stopPropagation(); handleOpenModal(product) }} className="p-2 bg-white rounded-full text-slate-700 hover:text-primary transition-colors"><Icon name="edit" /></button>
                    <button onClick={(e) => { e.stopPropagation(); handleDelete(product.id) }} className="p-2 bg-white rounded-full text-slate-700 hover:text-red-500 transition-colors"><Icon name="delete" /></button>
                  </div>
                </div>
              ...
              <div className="p-6 overflow-y-auto space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <label className="border-2 border-dashed border-slate-300 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-slate-50 transition-colors h-32 relative overflow-hidden group cursor-pointer">
                      {formData.image ? (
                        <>
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={formData.image} alt="" className="h-full w-full object-contain" />
                          <button onClick={(e) => { e.preventDefault(); setFormData({ ...formData, image: '' }) }} className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"><Icon name="close" className="text-sm" /></button>
                        </>
                      ) : (
                        <>
                  ...
                  <div>
                    ...
                    <label className="border-2 border-dashed border-slate-300 rounded-xl p-4 flex flex-col items-center justify-center text-center hover:bg-slate-50 transition-colors h-32 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PHBhdGggZD0iTTEwIDBoMTB2MTBIMTBWMHkwIDEwaDEwdjEwSDBWMHoiIGZpbGw9IiNmOGZhZmMiIGZpbGwtcnVsZT0iZXZlbm9kZCIvPjwvc3ZnPg==')] relative overflow-hidden group cursor-pointer">
                      {formData.imageNoBg ? (
                        <>
                          {/* eslint-disable-next-line @next/next/no-img-element */}
                          <img src={formData.imageNoBg} alt="" className="h-full w-full object-contain" />
                          <button onClick={(e) => { e.preventDefault(); setFormData({ ...formData, imageNoBg: '' }) }} className="absolute top-2 right-2 bg-black/50 text-white rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"><Icon name="close" className="text-sm" /></button>
                        </>
                      ) : (
                        <>
                      ...
                      <input type="file" accept="image/png" onChange={(e) => handleImageUpload(e, 'imageNoBg')} className="hidden" />
                    </label>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-4 overflow-x-auto pb-2">
                    ...
                    <label className="size-24 shrink-0 rounded-lg border-2 border-dashed border-slate-300 flex flex-col items-center justify-center text-slate-400 hover:text-primary hover:border-primary hover:bg-slate-50 transition-all cursor-pointer">
