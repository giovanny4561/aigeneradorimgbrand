...
export const ViewBranding: React.FC<Props> = ({ onComplete }) => {
  const [step, setStep] = useState<'upload' | 'analyzing' | 'editor'>('upload');
  const [brandData, setBrandData] = useState<BrandConfig>({
    name: 'Acme Corp',
    website: '',
    primaryColor: '#8b5cf6', // Default Lilac/Violet
  ...
  const handleAnalyze = () => {
    ...
    setTimeout(() => {
        ...
        setBrandData(prev => ({
            ...
            fontHeading: 'Inter',
            logo: 'https://lh3.googleusercontent.com/aida-public/AB6AXuBGmbIkwDsHjJNpxy-F0tp8WLJPdUd2LeYMeJRkyDecjHMMfGW0ogGQzQUTfg9moPzPkU-H31XqkaGcshR0TOp3xMFlgz4iBL2xvHSHmgqHkPGekelF1M2lh22kN_W_MhVC3CbgGHZHcwhyeOSQ4jh66hEe5wbDXxR-BNB2Gpl25DzFsZrXfPLTtXcgMhehdgaicKIuUVyZGDZoRrv713XoC3KfL3WOpVWRvswn9EXQepqfjI87fnQUraIfabVrYnKTLj6SVFMtX2cv'
        }));
    }, 2000);
  };
        ...
        <div className="flex-1 h-full overflow-y-auto bg-slate-50 p-6">
            <div className="max-w-6xl mx-auto h-full flex flex-col">
                ...
                <div className="flex items-center justify-between mb-8">
                    ...
                    <button onClick={onComplete} className="flex items-center gap-2 rounded-xl bg-primary px-6 py-3 text-sm font-bold text-white shadow-lg shadow-primary/20 hover:bg-primary-hover transition-all">
                         ...
                         <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm">
                            ...
                            <div className="flex items-center gap-4">
                                <div className="size-16 rounded-lg border border-slate-200 p-2 flex items-center justify-center bg-slate-50">
                                    {brandData.logo ? <img src={brandData.logo} className="max-w-full max-h-full object-contain" /> : <Icon name="image" className="text-slate-300" />}
                                </div>
                                <button className="text-sm text-primary font-medium hover:underline">Cambiar archivo</button>
                            </div>
                        </div>
                             ...
                             <div className="relative w-[380px] bg-white rounded-xl shadow-2xl overflow-hidden transform transition-all duration-300">
                                 ...
                                 <div className="p-6">
                                     ...
                                     <p className="text-slate-500 text-sm leading-relaxed mb-6" style={{fontFamily: brandData.fontBody}}>
                                         Descubre la nueva colección inspirada en la esencia minimalista de {brandData.name}. Calidad premium, diseño atemporal.
                                     </p>
                                 ...
                                 <div className="px-6 py-4 bg-slate-50 border-t border-slate-100 flex items-center justify-between">
                                     <div className="flex items-center gap-2">
                                        {brandData.logo && <img src={brandData.logo} className="h-6 w-auto" />}
                                        <span className="text-xs font-semibold text-slate-700">{brandData.name}</span>
                                     </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <label className="flex flex-col gap-2">
                        ...
                        <input 
                            ...
                            className="h-14 px-4 rounded-xl bg-white border border-slate-200 text-text-main placeholder-slate-400 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all shadow-sm" placeholder="ej. Acme Corp" type="text"/>
                    </label>
                    <label className="flex flex-col gap-2">
                        <span className="text-sm font-semibold text-text-main">URL del Sitio Web</span>
                        <div className="relative">
                            <input className="w-full h-14 px-4 pr-10 rounded-xl bg-white border border-slate-200 text-text-main placeholder-slate-400 focus:ring-2 focus:ring-primary focus:border-transparent outline-none transition-all shadow-sm" placeholder="https://..." type="url"/>
                ...
                <div>
                    ...
                    <div className="flex items-center gap-6 p-6 rounded-2xl border border-slate-200 bg-white shadow-sm">
                        <div className="relative group cursor-pointer shrink-0">
                            <div className="size-32 rounded-full border-2 border-dashed border-primary/40 bg-primary-light/30 flex flex-col items-center justify-center hover:bg-primary-light hover:border-primary transition-all duration-300">
                        ...
                        <div className="flex flex-col gap-1">
                            ...
                            <p className="text-sm text-text-muted">Sube un PNG o SVG de alta resolución. <br/>Tamaño recomendado: 512x512px.</p>
                        </div>
                    </div>
                </div>
